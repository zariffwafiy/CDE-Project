...
def construct_prompt(word1, word2, word3):
    ...
    prompt = (
        f"{definitions}\n{context}\n"
        f"Provide a precise semantic similarity and letter composition score (0.xxxx) between the data attribute '{word1}' in the data dictionary and the data element '{word2}' in the data standard. '{word3}' is the precise definition of the data element. Emphasize the exact, literal meanings. Penalize generalized semantic meaning matching and unrelated matching with lower scores. Similar letter composition shoiuld also be rewarded with higher scores.\n\n"
        f"{example1}\n{example2}"
    )
...
def construct_prompt_v2(word1, word2, word3):
    definitions = (
...
def openai_similarity_v2(word1, word2, word3, group, entity):
    try:
        prompt = construct_prompt_v2(word1, word2, word3)
        ...
        if model == "OpenAI":
            ...
            if st.button("Compare"):
                ...
                with ThreadPoolExecutor(max_workers = 2) as executor:
                    ...
                    scores = list(executor.map(lambda x: openai_similarity_v2(data_input, x[0], x[1], x[2], x[3]), standard_info))
