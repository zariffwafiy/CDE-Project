...
def process_word_pair(word1, data2, sheet_df_1, sheet_df_2, sheet_df_comb, summary_sheet):
    ...
    data_element_scores = data2["DATA ELEMENT"].apply(
        lambda data_element: round(openai_similarity(word1, data_element), 4)
    )

    # Check if the column already exists
    if original_name in sheet_df_1.columns:
        # If the column exists, update the values
        sheet_df_1[original_name] += data_element_scores
    else:
        # If the column doesn't exist, create a new column
        sheet_df_1[original_name] = data_element_scores

    # Compare field_desc_vs_glossary
    glossary_scores = data2["BUSINESS DEFINITION/ GLOSSARY"].apply(
        lambda glossary: round(openai_similarity(word1, glossary), 4)
    )

    # Check if the column already exists
    if original_name in sheet_df_2.columns:
        # If the column exists, update the values
        sheet_df_2[original_name] += glossary_scores
    else:
        # If the column doesn't exist, create a new column
        sheet_df_2[original_name] = glossary_scores
...
def main():
    ...
    data1 = data1[~(data1["FIELD DESCRIPTION"].str.contains(exclude_pattern, case=False, na=False) | data1["FIELD DESCRIPTION"].isna())].reset_index().head(4)
    ...
    output_path = "results/result_openai_filtervalue_specdomain_2.xlsx"
    with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer, ProcessPoolExecutor(max_workers=4) as executor:
