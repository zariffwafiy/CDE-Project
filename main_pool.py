...
def compare_and_get_top_matches(args, sheet_df_1, sheet_df_2, sheet_df_comb, summary_sheet):
    field_desc, original_name, data2 = args
...
def main():
    ...
    data1 = data1[~(data1["FIELD DESCRIPTION"].str.contains(exclude_pattern, case=False, na=False) | data1["FIELD DESCRIPTION"].isna())].reset_index().head(2)
    ...
    with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
        args_list = [(field_desc, original_name, data2) for field_desc, original_name in zip(data1["FIELD DESCRIPTION"], data1_names)]
        
        with Pool() as pool:
            results = pool.map(compare_and_get_top_matches, args_list, sheet_df_1, sheet_df_2, sheet_df_comb, summary_sheet)

        summary_sheet = pd.concat([summary_sheet.dropna(), pd.DataFrame(results).dropna()], ignore_index=True)
